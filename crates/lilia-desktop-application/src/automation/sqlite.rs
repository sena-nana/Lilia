...
impl SqliteAutomationStore {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, AutomationStoreError> {
        let path = path.as_ref();
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|error| {
                AutomationStoreError::storage("create database directory", error)
            })?;
        }
    ...
    pub fn from_connection(mut connection: Connection) -> Result<Self, AutomationStoreError> {
        connection
            .busy_timeout(Duration::from_secs(5))
            .map_err(|error| AutomationStoreError::storage("configure busy timeout", error))?;
        migrate_cancelled_status(&mut connection)?;
        connection
            .pragma_update(None, "foreign_keys", "ON")
            .map_err(|error| AutomationStoreError::storage("enable foreign keys", error))?;
        connection
            .execute_batch(SCHEMA)
            .map_err(|error| AutomationStoreError::storage("initialize schema", error))?;

        let conflicts = existing_active_run_conflicts(&connection)?;
        if !conflicts.is_empty() {
            return Err(AutomationStoreError::ExistingActiveRunConflict { conflicts });
        }
        connection
            .execute_batch(ACTIVE_RUN_INDEX)
            .map_err(|error| {
...
impl AutomationStore for SqliteAutomationStore {
    ...
    fn workflow(
        ...
        workflow_on(&self.connection, workflow_id)
    }
    ...
    fn save_draft(
        ...
        workflow_on(&self.connection, &workflow_id)?.ok_or_else(|| {
    ...
    fn version(
        ...
        version_on(&self.connection, version_id)
    }
    ...
    fn run_detail(
        ...
        let Some(run) = run_on(&self.connection, run_id)? else {
            return Ok(None);
        };
        let nodes = run_nodes_on(&self.connection, run_id)?;
    ...
    fn apply_execution_transition(
        ...
        self.run_detail(&transition.run_id)?
            .ok_or_else(|| AutomationStoreError::SchemaInvariant {
                message: format!(
                    "run {} disappeared after execution transition",
                    transition.run_id
                ),
            })
    }
...
mod tests {
    ...
    use super::*;
    use crate::automation::{AutomationEdge, AutomationNode, AutomationNodePosition};
    ...
    fn existing_status_schema_is_migrated_without_losing_run_data() {
        ...
        let mut store = SqliteAutomationStore::from_connection(connection).unwrap();
