...
pub(crate) struct DesktopApplicationInner {
    ...
    pub(crate) update_state: Mutex<crate::DesktopUpdateState>,
...
pub struct DesktopTaskSessionSnapshot {
    ...
    pub task_todos: Vec<crate::DesktopTaskTodo>,
    pub worktree: Option<crate::DesktopTaskWorktree>,
...
impl DesktopApplication {
    pub fn bootstrap(
        ...
        let credentials = crate::provider::persistent_credential_bridge(&config, host.clone())?;
        let authority =
    ...
    pub fn from_authority_with_memory_settings(
        ...
        .map_err(|error| crate::DesktopProviderError::Persistence(error.to_string()))?;
        let provider_settings =
            crate::provider::DesktopAgentRuntimeSettingsState::open(provider_settings_store)?;
        authority
            .shared_runtime()
            .inner()
        ...
        .map_err(|error| crate::DesktopAgentInteractionError::Persistence(error.to_string()))?;
        let agent_interaction =
            crate::agent_interaction::DesktopAgentInteractionState::open(agent_interaction_store)?;
        authority
            .shared_runtime()
            .inner()
            .configure_subagents(
                crate::agent_interaction::DesktopAgentInteractionState::runtime_definitions(
                    &agent_interaction.settings(),
                    &agent_interaction.agents,
                ),
            )
            .map_err(|error| crate::DesktopAgentInteractionError::RuntimeApply {
        ...
        Ok(Self {
            inner: Arc::new(DesktopApplicationInner {
                ...
                architecture,
                remote,
                update_state: Mutex::new(crate::DesktopUpdateState::Idle),
    ...
    pub fn query_tasks(
        ...
        match query.scope {
            crate::DesktopTaskScope::All => {}
            crate::DesktopTaskScope::Project(project_id) => {
                tasks.retain(|task| task.project_id.as_ref() == Some(&project_id));
            }
            crate::DesktopTaskScope::Inbox => {
...
pub enum DesktopApplicationError {
    ...
    #[error(transparent)]
    PanelLayout(#[from] crate::PanelLayoutError),
    #[error(transparent)]
    WorkspaceSession(#[from] crate::DesktopWorkspaceSessionIdError),
    #[error(transparent)]
    WorkspaceSessionState(#[from] crate::DesktopWorkspaceSessionStateError),
    #[error(transparent)]
    WorkspaceItem(#[from] crate::WorkspaceItemError),
    #[error(transparent)]
    Document(#[from] crate::DocumentError),
    #[error(transparent)]
    Language(#[from] crate::LanguageRegistryError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    AutoTurnDecision(#[from] crate::DesktopAutoTurnDecisionError),
    #[error(transparent)]
    ...
    #[error(transparent)]
    Automation(#[from] crate::DesktopAutomationError),
    #[error(transparent)]
    Memory(#[from] crate::DesktopMemoryError),
    #[error(transparent)]
    Roadmap(#[from] crate::RoadmapStoreError),
    #[error(transparent)]
    Architecture(#[from] crate::DesktopArchitectureError),
    #[error(transparent)]
    RemoteControl(#[from] crate::DesktopRemoteControlError),
    #[error(transparent)]
    Provider(#[from] crate::DesktopProviderError),
    #[error(transparent)]
    AgentInteraction(#[from] crate::DesktopAgentInteractionError),
    #[error(transparent)]
    Hook(#[from] crate::DesktopHookError),
...
mod tests {
    ...
    use super::*;
    use crate::{DesktopCredentialAction, DesktopHostResult, DesktopSecret, DesktopWindowAction};
...

        {
            let app =
                DesktopApplication::bootstrap(config.clone(), Arc::new(RecordingHost::default()))
                    .unwrap();
            let project = app
                .create_project(crate::DesktopProjectCreate::new("Native project"))
                .unwrap();
            let task = app
                .create_task(crate::DesktopTaskCreate::new(
            ...
            let memory = app
                .save_memory(crate::MemoryUpsertInput {
                    id: None,
    ...
    fn task_turn_runs_through_service_wire_and_refreshes_authoritative_projection() {
        ...
        let dispatch = app
            .start_task_turn(crate::DesktopTurnRequest::new(
                task_id.clone(),
                "complete this turn",
            ))
            .unwrap();
        assert_eq!(dispatch.kind, crate::DesktopTurnDispatchKind::Started);
        ...
        while Instant::now() < deadline {
            ...
            if matches!(
                ...
                DesktopEventKind::TurnStateChanged {
                    ref task_id,
                    state: crate::DesktopTurnState::Completed,
                    ..
    ...
    fn restarted_application_restores_and_rejects_a_native_permission() {
        ...
        app.start_task_turn(crate::DesktopTurnRequest::new(
            task_id.clone(),
            "ask before writing",
        ))
        .unwrap();
        ...
        while Instant::now() < deadline {
            ...
            if matches!(
                ...
                DesktopEventKind::TurnStateChanged {
                    ref task_id,
                    state: crate::DesktopTurnState::WaitingApproval { .. },
                    ..
            ...
            if let DesktopEventKind::TurnStateChanged {
                ...
                state: crate::DesktopTurnState::Failed { message },
                ..
            } = event.kind
            {
        ...
        while Instant::now() < deadline {
            ...
            match event.kind {
                DesktopEventKind::TurnStateChanged {
                    ref task_id,
                    state: crate::DesktopTurnState::Cancelled,
                    ..
                } if task_id.as_str() == "task-native-rejection" => {
                    cancelled = true;
                    break;
                ...
                DesktopEventKind::TurnStateChanged {
                    ref task_id,
                    state: crate::DesktopTurnState::Failed { ref message },
                    ..
                } if task_id.as_str() == "task-native-rejection" => {
