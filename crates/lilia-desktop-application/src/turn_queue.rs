...
pub(crate) enum PersistedDesktopTurnState {
    Queued,
    Claimed,
}
...
pub(crate) struct PersistedDesktopTurn {
    pub turn_id: String,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct PersistedDesktopTurnDebugState {
    pub turn_id: String,
    pub state: String,
    pub claim_epoch: Option<String>,
    pub claim_attempts: u64,
...
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct QuarantinedDesktopTurn {
    pub task_id: String,
    pub turn_id: String,
...
pub(crate) struct DesktopTurnQueueStore {
    connection: SharedLegacyConnection,
}

impl DesktopTurnQueueStore {
    #[cfg(test)]
    pub(crate) fn open(path: &Path) -> Result<Self, DesktopTurnQueueError> {
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|error| DesktopTurnQueueError::Storage {
                operation: "create turn queue database directory",
                message: error.to_string(),
            })?;
        }
        let connection =
            Connection::open(path).map_err(|error| DesktopTurnQueueError::Storage {
    ...
    #[cfg(test)]
    pub(crate) fn in_memory() -> Result<Self, DesktopTurnQueueError> {
        let connection =
            Connection::open_in_memory().map_err(|error| DesktopTurnQueueError::Storage {
                operation: "open in-memory turn queue database",
                message: error.to_string(),
            })?;
        Self::from_connection(connection)
    }
    ...
    #[cfg(test)]
    fn from_connection(connection: Connection) -> Result<Self, DesktopTurnQueueError> {
    ...
    pub(crate) fn from_shared(
        ...
        let locked = connection
            .lock()
            .map_err(|_| DesktopTurnQueueError::Storage {
                operation: "lock turn queue database for initialization",
    ...
    pub(crate) fn enqueue(
        ...
        let connection = self.connection("enqueue pending turn")?;
    ...
    pub(crate) fn enqueue_idempotent(
        ...
        let connection = self.connection("idempotently enqueue pending turn")?;
    ...
    pub(crate) fn update_request(
        ...
        let changed = self
            .connection("update pending turn request")?
            .execute(
    ...
    pub(crate) fn claim(
        ...
        let mut connection = self.connection("begin pending turn claim")?;
    ...
    pub(crate) fn quarantine_invalid_rows(
        &mut self,
    ) -> Result<Vec<QuarantinedDesktopTurn>, DesktopTurnQueueError> {
        let mut connection = self.connection("begin invalid pending turn quarantine")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn list_quarantined(
        &self,
    ) -> Result<Vec<QuarantinedDesktopTurn>, DesktopTurnQueueError> {
        let connection = self.connection("list quarantined turns")?;
    ...
    #[cfg(debug_assertions)]
    pub(crate) fn corrupt_request_for_debug(
        &self,
        turn_id: &str,
    ) -> Result<bool, DesktopTurnQueueError> {
        self.connection("inject invalid pending turn request")?
            .execute(
                r#"UPDATE desktop_pending_turns
    ...
    pub(crate) fn ack_and_claim_next(
        &mut self,
        task_id: &TaskId,
        turn_id: &str,
        claim_token: &str,
        ...
        let mut connection = self.connection("begin pending turn acknowledgement")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn claim_first(
        ...
        let mut connection = self.connection("begin first pending turn claim")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn discard_queued_and_claim_next(
        &mut self,
        task_id: &TaskId,
        turn_id: &str,
        claim_epoch: &str,
    ) -> Result<Option<PersistedDesktopTurn>, DesktopTurnQueueError> {
        let mut connection = self.connection("begin prepared turn discard")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn prepare_recovery(
        ...
        let mut connection = self.connection("begin pending turn recovery")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn remove(&self, turn_id: &str) -> Result<bool, DesktopTurnQueueError> {
        self.connection("remove pending turn")?
            .execute(
    ...
    pub(crate) fn contains(&self, turn_id: &str) -> Result<bool, DesktopTurnQueueError> {
        self.connection("inspect pending turn")?
            .query_row(
    ...
    pub(crate) fn discard_queued(
        ...
        self.connection("discard queued turn")?
            .execute(
                r#"DELETE FROM desktop_pending_turns
                   WHERE task_id = ?1 AND turn_id = ?2 AND state = 'queued'"#,
                params![task_id.as_str(), turn_id],
    ...
    pub(crate) fn cancel_claim_and_clear_task(
        &mut self,
        task_id: &TaskId,
        turn_id: &str,
        claim_token: Option<&str>,
    ) -> Result<usize, DesktopTurnQueueError> {
        let mut connection = self.connection("begin task turn cancellation")?;
        let transaction = connection
            .transaction()
    ...
    pub(crate) fn list_task_ids(&self) -> Result<Vec<TaskId>, DesktopTurnQueueError> {
        let connection = self.connection("list pending turn task ids")?;
        let mut statement = connection
    ...
    pub(crate) fn list(
        ...
        let connection = self.connection("list pending turns")?;
        let mut statement = connection
            .prepare(
    ...
    #[cfg(debug_assertions)]
    pub(crate) fn list_debug(
        &self,
        task_id: &TaskId,
    ) -> Result<Vec<PersistedDesktopTurnDebugState>, DesktopTurnQueueError> {
        let connection = self.connection("list pending turns for debug")?;
        let mut statement = connection
            .prepare(
    ...
    pub(crate) fn clear_task(&self, task_id: &TaskId) -> Result<usize, DesktopTurnQueueError> {
        self.connection("clear task pending turns")?
            .execute(
                "DELETE FROM desktop_pending_turns WHERE task_id = ?1",
                params![task_id.as_str()],
            )
    ...
    ) -> Result<std::sync::MutexGuard<'_, Connection>, DesktopTurnQueueError> {
        self.connection
            .lock()
            .map_err(|_| DesktopTurnQueueError::Storage {
                operation,
...
mod tests {
    ...
    fn corrupt_row_is_quarantined_without_blocking_remaining_fifo_rows() {
        ...
        store
            .connection("inject corrupt pending turn")
            .unwrap()
            .execute(
        ...
        let raw_request: String = store
            .connection("read quarantined raw request")
            .unwrap()
            .query_row(
    ...
    fn direct_claim_validates_before_committing_claim_state() {
        ...
        let state: String = store
            .connection("inspect failed pending turn claim")
            .unwrap()
            .query_row(
