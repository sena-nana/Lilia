...
impl SqliteArchitectureStore {
    pub fn open(path: impl AsRef<Path>) -> Result<Self, DesktopArchitectureError> {
        ...
        if let Some(parent) = path.parent() {
            std::fs::create_dir_all(parent).map_err(|error| {
                DesktopArchitectureError::storage("create database directory", error)
            })?;
        }
        let connection = Connection::open(path)
            .map_err(|error| DesktopArchitectureError::storage("open database", error))?;
    ...
    pub fn from_connection(connection: Connection) -> Result<Self, DesktopArchitectureError> {
        connection
            .busy_timeout(Duration::from_secs(5))
            .map_err(|error| DesktopArchitectureError::storage("configure busy timeout", error))?;
        connection
