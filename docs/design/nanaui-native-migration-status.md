...
- [x] **Title update coordinator**：`DesktopTitleUpdateCoordinator` 与 `spawn_title_update_after_turn` 在应用层；turn Completed 后自动调度；Tauri `spawn`/`respond` 薄适配共享权威；标题 source 存 agent-runtime，不双写 Tauri legacy tasks 表。
