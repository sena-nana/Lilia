...
const ENABLE_ENV: &str = "LILIA_NATIVE_AGENT_DEBUG";
const ADDRESS_ENV: &str = "LILIA_NATIVE_AGENT_DEBUG_ADDR";
const READY_ENV: &str = "LILIA_NATIVE_AGENT_DEBUG_READY";
...
mod tests {
    ...
    fn parses_supported_jsonl_commands_and_escapes() {
        ...
        assert_eq!(
            parse_request(r#"{"command":"click","targetId":"native-preview.settings.open"}"#)
                .unwrap(),
            DebugCommand::Click {
                target_id: "native-preview.settings.open".to_owned()
            }
        );
        assert_eq!(
        ...
        assert_eq!(
            parse_request(
                r#"{"command":"input-frame","targetId":"native-preview.task-session.composer.input","text":"frame"}"#
            )
            .unwrap(),
            DebugCommand::InputFrame {
                target_id: "native-preview.task-session.composer.input".to_owned(),
                text: "frame".to_owned(),
            }
        );
        assert_eq!(
    ...
    fn responses_are_valid_jsonl_without_raw_control_characters() {
        let observation = DebugObservation {
            ...
            workspace_session_id: "native-preview.primary".to_owned(),
            ...
            task_popup_session_ids: vec!["native-preview.popup.task.one.100".to_owned()],
            ...
            visible_target_ids: vec!["native-preview.projects".to_owned()],
