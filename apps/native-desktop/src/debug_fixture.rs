...
pub fn prepare(application: &DesktopApplication) -> Result<(), String> {
    if std::env::var("LILIA_NATIVE_AGENT_DEBUG_SEED").as_deref() != Ok("1") {
        return Ok(());
    }
    ...
    if let Ok(workspace) = std::env::var("LILIA_NATIVE_AGENT_DEBUG_WORKSPACE") {
            ...
            .apply_project_architecture(ProjectArchitectureApplyInput {
                ...
                changes: vec![
                    ProjectArchitectureChange::UpsertNode {
                        node: node("native-ui", "NanaUI Workspace", "apps/native-desktop"),
                    },
                    ProjectArchitectureChange::UpsertNode {
                        node: node(
                            "desktop-application",
    ...
    let endpoint = std::env::var("LILIA_NATIVE_AGENT_DEBUG_MODEL_ENDPOINT")
        .map_err(|_| "LILIA_NATIVE_AGENT_DEBUG_MODEL_ENDPOINT is required for seeded replay")?;
...
fn seed_corrupt_architecture_snapshot(
    ...
    if std::env::var("LILIA_NATIVE_AGENT_DEBUG_CORRUPT_ARCHITECTURE").as_deref() != Ok("1") {
        return Ok(());
    }
    let connection =
        rusqlite::Connection::open(application.config().data_paths().legacy_desktop_db())
...
fn seed_extensions(application: &DesktopApplication) -> Result<(), String> {
    ...
    let workspace = std::env::var("LILIA_NATIVE_AGENT_DEBUG_WORKSPACE")
