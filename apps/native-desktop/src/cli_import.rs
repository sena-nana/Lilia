...
use crate::storage::{preview_home, preview_instance_identity};
...
pub fn run(command: ImportCommand) -> Result<CliResult, String> {
    let target = preview_config(preview_home()?)?;
    let service = DesktopDataImportService::new(target, Arc::new(PreviewHost));
    match command {
        ImportCommand::Plan { source, output } => {
            let source = DesktopApplicationConfig::new(source, LEGACY_INSTANCE_IDENTITY)
...
fn preview_config(home: impl Into<PathBuf>) -> Result<DesktopApplicationConfig, String> {
    let home = home.into();
    let instance_identity = preview_instance_identity(&home)?;
    DesktopApplicationConfig::new(home, instance_identity).map_err(|error| error.to_string())
}
...
mod tests {
    ...
    fn plan_file_roundtrips_and_execute_denies_credentials_without_host_access() {
        ...
        let source = DesktopApplicationConfig::new(&source_home, LEGACY_INSTANCE_IDENTITY).unwrap();
        let target = preview_config(&target_home).unwrap();
        let service = DesktopDataImportService::new(target, Arc::new(PreviewHost));
    ...
    fn confirmed_empty_credential_manifest_completes_without_keyring_changes() {
        let root = temporary_root();
        let source =
            DesktopApplicationConfig::new(root.join("source"), LEGACY_INSTANCE_IDENTITY).unwrap();
        let target = preview_config(root.join("target")).unwrap();
        let service = DesktopDataImportService::new(target, Arc::new(PreviewHost));
        let plan = service.plan(&source).unwrap();
        let report = service.execute(
            &plan,
