...
const DEFAULT_PREVIEW_ENDPOINT: &str =
    "https://github.com/sena-nana/LiliaCode/releases/download/native-preview/latest-native-preview.json";
pub const PREVIEW_RELEASES_URL: &str = "https://github.com/sena-nana/LiliaCode/releases";
const USER_AGENT: &str = concat!("LiliaCode-Native-Preview/", env!("CARGO_PKG_VERSION"));
...
pub fn execute_with_progress(
    ...
    match action {
        ...
        DesktopUpdateAction::Install { version } => {
            let config = NativeUpdaterConfig::embedded(PREVIEW_CHANNEL)?;
...
pub fn is_configured() -> bool {
    option_env!("LILIA_NATIVE_UPDATER_PUBKEY").is_some_and(|value| !value.trim().is_empty())
}

impl NativeUpdaterConfig {
    fn embedded(channel: &str) -> Result<Self, DesktopHostError> {
        if channel.trim() != PREVIEW_CHANNEL {
            return Err(update_error(
                "native_updater_channel_invalid",
                "Native Preview 仅支持独立的 Preview 更新通道。",
                false,
            ));
        }
        let public_key = option_env!("LILIA_NATIVE_UPDATER_PUBKEY")
            .unwrap_or_default()
            .trim()
            .to_owned();
        if public_key.is_empty() {
            return Err(update_error(
                ...
                "此构建未配置 Native Preview 更新签名，自动更新不可用。",
                false,
            ));
        }
        let endpoint = option_env!("LILIA_NATIVE_UPDATER_ENDPOINT")
            .unwrap_or(DEFAULT_PREVIEW_ENDPOINT)
            .trim();
...
fn stage_installer(
    ...
    let result = if package.starts_with(b"PK\x03\x04") {
        ...
        let name = format!("LiliaCodeNativePreview_{version}_x64-setup.exe");
...
fn launch_installer(_path: &Path) -> Result<(), DesktopHostError> {
    Err(update_error(
        ...
        "Native Preview 自动更新当前仅支持 Windows 11。",
        false,
    ))
}
...
mod tests {
    ...
    fn static_manifest_prefers_the_nsis_target_and_normalizes_v_prefix() {
        ...
        assert_eq!(release.notes.as_deref(), Some("Native Preview release"));
    }
    ...
    #[test]
    fn direct_nsis_package_is_staged_below_the_preview_home() {
        ...
        let context = DesktopHostContext {
            home: home.clone(),
            instance_identity: "liliacode.native-preview.test".into(),
        };
    ...
    fn updater_context(label: &str) -> DesktopHostContext {
        DesktopHostContext {
            ...
            instance_identity: format!("liliacode.native-preview.{label}"),
        }
    }
