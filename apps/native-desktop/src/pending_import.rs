...
pub fn discard_staging(base_home: &Path, staging_home: &Path) -> Result<(), String> {
    if !staging_home.exists() {
        ...
        return Err("staged import is outside the Native Preview import directory".to_owned());
    }
...
pub fn launch_helper() -> Result<(), String> {
    let executable = std::env::current_exe()
        .map_err(|error| format!("cannot locate Native Preview executable: {error}"))?;
...
pub fn run_helper(base_home: &Path) -> Result<(), String> {
    ...
    if activation.is_err() {
        command.env("LILIA_NATIVE_IMPORT_ACTIVATION_FAILED", "1");
    }
    command
        .spawn()
        .map_err(|error| format!("cannot restart Native Preview: {error}"))?;
    activation
}
...
fn activate(base_home: &Path) -> Result<(), String> {
    ...
    if !target_is_empty(base_home)? {
        return Err(
            "Native Preview already contains data; pending import was not activated".to_owned(),
        );
    }
    let staging_paths = LiliaDataPaths::from_home(&staging_home);
...
fn wait_for_writer(path: &Path) -> Result<File, String> {
    ...
    loop {
        let file = OpenOptions::new()
            ...
            .create(false)
            .open(path)
            .map_err(|error| format!("cannot open Native Preview writer lock: {error}"))?;
        match file.try_lock_exclusive() {
            ...
            Err(error) => {
                return Err(format!(
                    "Native Preview did not release its writer lock in time: {error}"
                ));
            }
        }
    }
...
fn database_contains_user_data(path: &Path) -> Result<bool, String> {
    ...
    .map_err(|error| format!("cannot inspect Native Preview data: {error}"))?;
    let mut statement = connection
        .prepare(
            "SELECT name FROM sqlite_master \
             WHERE type = 'table' AND name NOT LIKE 'sqlite_%' ORDER BY name",
        )
        .map_err(|error| format!("cannot inspect Native Preview schema: {error}"))?;
...
mod tests {
    ...
    fn freshly_bootstrapped_native_application_remains_importable() {
        ...
        let application = DesktopApplication::bootstrap(config, Arc::new(PreviewHost)).unwrap();
        assert!(target_is_empty(&home).unwrap());
        drop(application);
        fs::remove_dir_all(home).unwrap();
    }
    ...
    fn created_staging_home_accepts_a_real_nonempty_database_import() {
        ...
        let source =
            DesktopApplication::bootstrap(source_config.clone(), Arc::new(PreviewHost)).unwrap();
        source
        ...
        let service = DesktopDataImportService::new(target_config, Arc::new(PreviewHost));
