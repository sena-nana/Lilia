...
mod windows_launcher {
    ...
    const PRODUCT_NAME: &str = "LiliaCode";
    const HOST_LIBRARY: &str = "liliacode_host.dll";
    const HOST_ENTRYPOINT: &[u8] = b"liliacode_run\0";
    ...
    fn lilia_home() -> Result<PathBuf, String> {
        let paths = lilia_storage::LiliaDataPaths::resolve();
        paths
            .ensure_layout()
            .map_err(|error| format!("failed to prepare LILIA_HOME: {error}"))?;
    ...
    unsafe fn create_startup_window() -> Result<HWND, String> {
        unsafe {
            ...
            let class_name = HSTRING::from("LiliaCode.StartupWindow");
