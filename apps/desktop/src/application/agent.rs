...
impl DesktopAgentRuntime {
    ...
    fn begin_resolution(&self, task_id: &TaskId, turn_id: &str, wait: ActiveWait) -> bool {
        let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
        let Some(active) = state
            .tasks
            .get_mut(task_id.as_str())
    ...
    fn restore_wait(&self, task_id: &TaskId, turn_id: &str, wait: ActiveWait) {
        let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
...
impl ActiveTurn {
    fn projected_phase(&self) -> &'static str {
        match self.wait {
