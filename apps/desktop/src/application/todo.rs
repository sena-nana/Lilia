...
use crate::application::{DesktopApplication, DesktopApplicationError, DesktopEventKind, DesktopTurnDispatch};
...
impl DesktopTodoStore {
    ...
    pub(crate) fn from_shared(connection: Db) -> Result<Self, DesktopTodoError> {
    ...
    fn create(&self, input: DesktopTodoCreate) -> Result<DesktopTaskTodo, DesktopTodoError> {
    ...
    pub(crate) fn create_idempotent_in(
        connection: &Connection,
        id: &str,
        input: DesktopTodoCreate,
        source: DesktopTodoSource,
    ...
    pub(crate) fn set_guide_status_in(
        connection: &Connection,
        id: &str,
...
pub(crate) fn guide_message(todo: &DesktopTaskTodo) -> String {
    format!(
        "{LILIA_GUIDE_PREFIX}\n优先级：{}\n\n{}",
...
fn merge_todos_with_latest_projection(
