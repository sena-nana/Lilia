...
fn main() -> Result<(), String> {
    ...
    if options.samples < 5 || options.min_background_ms == 0 {
        return Err("samples must be at least 5 and min-background-ms must be positive".into());
    }
    let calibrated = calibrate(&options)?;
    let single_domain = run_case(&options, calibrated, LiliaRuntimeTopology::SingleDomain)?;
    let three_domains = run_case(
        &options,
        calibrated,
...
fn run_case(
    ...
    for sample in 0..options.samples {
        ...
        if topology == LiliaRuntimeTopology::SingleDomain {
